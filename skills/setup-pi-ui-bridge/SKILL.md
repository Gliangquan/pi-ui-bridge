...
### 4. Instruct the user to load the browser extension in Chrome
...
### 5. Start the Pi UI Bridge server in Pi
...
### 6. Instruct the user to connect the browser extension to the Pi Bridge
...
- In the extension popup, paste the `bridgeUrl` and `token` obtained from Pi's `/ui-start` output.
...
### 7. (Optional) Start the React demo project
...
- **In Pi:** You can run `/ui-status` to check the bridge's state and recent activity.
